...
string CStatistics::show(HEADER header, string kind) {
    ...
    string out;
    out = "Total: " + clb::int2str(vVigent->size()) + "\n";
    vtPairInt::iterator it;
    if(kind == "quantity") {
        int i = 0;
...
string CStatistics::advanced_search(const string &nick, const HEADER &header) {
    CPairInt *apelido;
    vtPairInt::iterator it;
	...
	vtMsg::iterator it2;
  	for(it2 = vArticlesGeral->begin(); it2 != vArticlesGeral->end(); it2++) {
	    string nick2 = (*it2)->get_header(header);
  	    string ng2 = (*it2)->get_header(ID_HEADER_GROUPS);
	    if(nick == nick2 && ng == ng2)
		out += clb::int2str(cont++) + " - \n" +  (*it2)->toString2() + "\n";
	}
	delete *it;
    }
    if(ngs->size() == 0) 
...
vtPairInt* CStatistics::retorna_ngs_inscritos(const string &nick, const HEADER &hdr) {
    vtPairInt *vpi = new vtPairInt();
    vtMsg::iterator it;
    for(it = vArticlesGeral->begin(); it != vArticlesGeral->end(); it++) {
	CArticle *msg = *it;
  	string name = msg->get_header(hdr);  		
	if(nick == name) {		  
	    string value = msg->get_header(ID_HEADER_GROUPS);
	    CPairInt *pi = new CPairInt(new string(value), 1);
	    vpi->push_back(pi);
	}	  
    }
    put_together(vpi);
...
string CStatistics::postagem(const string &s) {
	...
	string hour2 = (*itm)->get_header(ID_HEADER_HOUR);
	if(hour2 == hour)
	    return (*itm)->toString();
    }
    return "NULL";
...
string CStatistics::top_happen(int qts, HEADER header) {
    ...
    int i = 0;
    for(it = vVigent->begin(); it < index; it++) {
	string ordem = clb::int2str(++i) + char(186) + "- " +  percentage((*it)->get_counter()) + (*it)->toString2() + "\n";
	out += ordem;
...
string CStatistics::pico(void) {
    ...
    for(it = vHour->begin(); it != vHour->end(); it++) {
	out = *(*it)->get_value();
	vpi->push_back(new CPairInt(new string(out.substr(0, 2)), 1));
    }
...
void CStatistics::define_partial(void) {
	...
	for(itm = vArticlesGeral->begin(); itm != vArticlesGeral->end(); itm++) {
	    CArticle *msg = *itm;
	    string group = msg->get_header(ID_HEADER_GROUPS);
...
string CStatistics::modelo1(const int &quantity)
{
    ...
    if(m_selection)
	out += "*****Os " + clb::int2str(quantity) + " maiores postadores do " + m_selected_ng.substr(pos) + "*****\n\n";
    else
	out += "*****Os " + clb::int2str(quantity) + " maiores postadores da net*****\n\n";
