...
CArticle::CArticle(/*vtPair *headers*/CPair *headers, bool ref, string body) {
//    cout << "CArticle()..." << countA++ << endl;
    m_thread  = ref;
    m_headers = headers;
    m_body    = body;
...
string CArticle::toString() const {
...
string CArticle::toString2() const {
...
string CArticle::get_header(const HEADER &hdr) const {
/*    vtPair::iterator it;
    for(it = m_headers->begin(); it != m_headers->end(); it++) {        
	CPair *pair = *it;
	if(pair->get_key() == hdr)
