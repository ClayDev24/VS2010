...
int WINAPI WinMain(	HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR lpcmdline, int ncmdshow)
{
	...
	{    
		...
		// Draws a colored point elsewhere upon client area:
		int x = rand()%cxClient;
		int y = rand()%cyClient;
		COLORREF randomColor = RGB(rand()%256,rand()%256,rand()%256);
		// set the color of the pixel at the specified position
		COLORREF crColor = SetPixel(hdc, x, y, randomColor);
		// return the color of the pixel at the specified position (x, y):
