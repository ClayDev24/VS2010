...
Panel::Panel( wxFrame *_frame, int x, int y, int w, int h )
       : wxPanel(_frame, wxID_ANY, wxPoint(x, y), wxSize(w, h) )
{
    m_listbox       = NULL;
...
static void SetListboxClientData(const wxChar *name, wxListBox *control) {
    ...
    for(size_t n = 0; n < count; n++) {
        wxString s;
        s.Printf(wxT("%s client data for '%s'"), name, control->GetString(n).c_str());
...
void Panel::OnPageChanging( wxBookCtrlEvent &event ) {
...
void Panel::OnPageChanged( wxBookCtrlEvent &event ) {
    *m_text << wxT("Book selection is now ") << event.GetSelection()
            << wxT(" (from book: ") << m_book->GetSelection()
            << wxT(")\n");
}

void Panel::OnTestButton(wxCommandEvent &event) {
    wxLogMessage(wxT("Button %c clicked."),
                 event.GetId() == ID_BUTTON_TEST1 ? wxT('1') : wxT('2'));
}

void Panel::OnBmpButton(wxCommandEvent &WXUNUSED(event)) {
    wxLogMessage(wxT("Bitmap button clicked."));
}

void Panel::OnBmpButtonToggle(wxCommandEvent &event) {
    FindWindow(ID_BITMAP_BTN)->Enable(!event.IsChecked());
}

void Panel::OnChangeColour(wxCommandEvent &WXUNUSED(event)) {
...
void Panel::OnListBox( wxCommandEvent &event ) {
...
void Panel::OnListBoxDoubleClick( wxCommandEvent &event ) {
    m_text->AppendText( wxT("ListBox double click string is: ") );
    m_text->AppendText( event.GetString() );
    m_text->AppendText( wxT("\n") );
}

void Panel::OnListBoxButtons( wxCommandEvent &event ) {
    switch (event.GetId()) {
...
void Panel::OnChoice( wxCommandEvent &event )
{
...
void Panel::OnChoiceButtons( wxCommandEvent &event ) {
    switch (event.GetId()) {
...
void Panel::OnCombo( wxCommandEvent &event ) {
    if(!m_combo)
        return;
...
void Panel::OnComboTextChanged(wxCommandEvent &event) {
    if(m_combo) {
...
void Panel::OnComboTextEnter(wxCommandEvent & WXUNUSED(event)) {
    if(m_combo)
        wxLogMessage(wxT("Enter pressed in the combobox: value is '%s'."),
                     m_combo->GetValue().c_str());
}

void Panel::OnComboButtons( wxCommandEvent &event ) {
    switch (event.GetId())
    {
...
void Panel::OnRadio( wxCommandEvent &event ) {
    m_text->AppendText( wxT("RadioBox selection string is: ") );
    m_text->AppendText( event.GetString() );
    m_text->AppendText( wxT("\n") );
}

void Panel::OnRadioButton1( wxCommandEvent &WXUNUSED(event) ) {
    wxMessageBox(wxT("First wxRadioButton selected."), wxT("wxControl sample"));
}

void Panel::OnRadioButton2( wxCommandEvent &WXUNUSED(event) ) {
    m_text->AppendText(wxT("Second wxRadioButton selected.\n"));
}

void Panel::OnRadioButtons( wxCommandEvent &event ) {
    switch (event.GetId()) {
...
void Panel::OnSetFont( wxCommandEvent &WXUNUSED(event) ) {
    m_fontButton->SetFont( *wxITALIC_FONT );
    m_text->SetFont( *wxITALIC_FONT );
}

void Panel::OnUpdateLabel( wxCommandEvent &event ) {
...
void Panel::OnSliderUpdate( wxCommandEvent &WXUNUSED(event) ) {
#if wxUSE_GAUGE
...
void Panel::OnSpinCtrlText(wxCommandEvent &event) {
    if(m_spinctrl) {
        wxString s;
...
void Panel::OnSpinCtrl(wxSpinEvent &event) {
    if(m_spinctrl) {
        wxString s;
...
void Panel::OnSpinCtrlUp(wxSpinEvent &event) {
    if(m_spinctrl) {
...
void Panel::OnSpinCtrlDown(wxSpinEvent& event) {
    if(m_spinctrl) {
        m_text->AppendText( wxString::Format(
...
#if wxUSE_SPINBTN
void Panel::OnSpinUp( wxSpinEvent &event ) {
    wxString value;
...
void Panel::OnSpinDown( wxSpinEvent &event ) {
    wxString value;
...
void Panel::OnSpinUpdate( wxSpinEvent &event ) {
    wxString value;
...
void Panel::OnNewText( wxCommandEvent & /* event */) {
...
void Panel::OnUpdateShowProgress( wxUpdateUIEvent &event ) {
    event.Enable( m_spinbutton->GetValue() > 0 );
}

void Panel::OnShowProgress( wxCommandEvent &WXUNUSED(event) ) {
...
void Panel::OnSizerCheck( wxCommandEvent &event) {
  switch (event.GetId ()) {
...
void Panel::listBoxChores() {
    ...
    (void)new Button(   panel, ID_LISTBOX_APPEND, wxT("&Append 'Hi!'"),      wxPoint(340,80),  wxSize(140,30) );
    (void)new wxButton( panel, ID_LISTBOX_DELETE, wxT("D&elete selected item"),  wxPoint(180,130), wxSize(140,30) );
    wxButton *button = new Button( panel, ID_LISTBOX_FONT, wxT("Set &Italic font"), wxPoint(340,130), wxSize(140,30) );
...
void Panel::choiceChores() {
    // ------------------------------------------------------------------------
    // choice page
    // ------------------------------------------------------------------------
#if wxUSE_CHOICE
    m_panel = new wxPanel(m_book);
    m_choice       = new Choice( m_panel, ID_CHOICE, wxPoint(10,10), wxDefaultSize, 5, choices );
    m_choiceSorted = new Choice( m_panel, ID_CHOICE_SORTED, wxPoint(10,70), wxSize(120,wxDefaultCoord),
                                   5, choices, wxCB_SORT );
    SetChoiceClientData(wxT("choice"), m_choice);
    SetChoiceClientData(wxT("choice"), m_choiceSorted);

    m_choice->SetSelection(2);
    (void)new wxButton( m_panel,  ID_CHOICE_SEL_NUM, wxT("Select #&2"),    wxPoint(220,30), wxSize(140,30) );
    (void)new wxButton( m_panel,  ID_CHOICE_SEL_STR, wxT("&Select 'This'"),  wxPoint(380,30), wxSize(140,30) );
    (void)new wxButton( m_panel,  ID_CHOICE_CLEAR,   wxT("&Clear"),            wxPoint(220,80), wxSize(140,30) );
    (void)new wxButton( m_panel,  ID_CHOICE_APPEND,  wxT("&Append 'Hi!'"),       wxPoint(380,80), wxSize(140,30) );
    (void)new wxButton( m_panel,  ID_CHOICE_DELETE,  wxT("D&elete selected item"), wxPoint(220,130), wxSize(140,30) );
    (void)new wxButton( m_panel,  ID_CHOICE_FONT,    wxT("Set &Italic font"),     wxPoint(380,130), wxSize(140,30) );
    (void)new wxCheckBox( m_panel, ID_CHOICE_ENABLE, wxT("&Disable"),          wxPoint(20,130), wxSize(140,30) );

    m_book->AddPage(m_panel, wxT("wxChoice"), false, Image_Choice);
#endif // wxUSE_CHOICE
}

void Panel::comboChores() {
    // ------------------------------------------------------------------------
    // combo page
    // ------------------------------------------------------------------------
    m_panel = new wxPanel(m_book);
    m_combo = new ComboBox( m_panel, ID_COMBO, wxT("This"),
                              wxPoint(15,25), wxDefaultSize,
                              5, choices,
                              wxTE_PROCESS_ENTER);
    wxSize combosize(m_combo->GetBestSize().x + 20, 100);
    (void)new wxStaticBox( m_panel, wxID_ANY, wxT("&Box around combobox"),
                           wxPoint(5, 5), combosize);

    (void)new wxButton(  m_panel, ID_COMBO_SEL_NUM,  wxT("Select #&2"),     wxPoint(220,30), wxSize(140,30) );
    (void)new wxButton(  m_panel, ID_COMBO_SEL_STR,  wxT("&Select 'This'"), wxPoint(380,30), wxSize(140,30) );
    (void)new wxButton(  m_panel, ID_COMBO_CLEAR,    wxT("&Clear"),         wxPoint(220,80), wxSize(140,30) );
    (void)new wxButton(  m_panel, ID_COMBO_APPEND,   wxT("&Append 'Hi!'"),  wxPoint(380,80), wxSize(140,30) );
    (void)new wxButton(  m_panel, ID_COMBO_DELETE,   wxT("D&elete selected item"), wxPoint(220,130), wxSize(140,30) );
    (void)new wxButton(  m_panel, ID_COMBO_FONT,     wxT("Set &Italic font"),  wxPoint(380,130), wxSize(140,30) );
    (void)new wxButton(  m_panel, ID_COMBO_SET_TEXT, wxT("Set 'Hi!' at #2"), wxPoint(380,180), wxSize(140,30) );
    (void)new wxCheckBox( m_panel, ID_COMBO_ENABLE,  wxT("&Disable"),       wxPoint(20,130), wxSize(140,30) );

    m_book->AddPage(m_panel, wxT("wxComboBox"), false, Image_Combo);
}

void Panel::radioBoxChores() {
	    // ------------------------------------------------------------------------
    // radio box
    // ------------------------------------------------------------------------
    wxString choices2[] = {
    ...
    m_radio = new wxRadioBox(m_panel, ID_RADIOBOX, wxT("T&his"),
                             wxPoint(10,10), wxDefaultSize,
                             WXSIZEOF(choices), choices,
                             1, wxRA_SPECIFY_COLS );
    RadioBox* mybox = new RadioBox(m_panel, ID_RADIOBOX2, wxT("&That"),
    ...
    wxButton  *select_two  = new wxButton  ( m_panel, ID_RADIOBOX_SEL_NUM, wxT("Select #&2") );
    wxButton  *select_this = new wxButton  ( m_panel, ID_RADIOBOX_SEL_STR, wxT("&Select 'This'") );
    m_fontButton           = new wxButton  ( m_panel, ID_SET_FONT,         wxT("Set &more Italic font") );
    wxButton   *set_italic = new wxButton  ( m_panel, ID_RADIOBOX_FONT,    wxT("Set &Italic font") );
    wxCheckBox *disable_cb = new wxCheckBox( m_panel, ID_RADIOBOX_ENABLE,  wxT("&Disable") );
    wxRadioButton *rb      = new wxRadioButton( m_panel, ID_RADIOBUTTON_1, wxT("Radiobutton1"),
                                                wxDefaultPosition, wxDefaultSize, wxRB_GROUP );
    wxRadioButton *rb2     = new wxRadioButton( m_panel, ID_RADIOBUTTON_2, wxT("&Radiobutton2"),
    ...
    m_panel->SetSizer( radio_page_sizer );

    m_book->AddPage(m_panel, wxT("wxRadioBox"), false, Image_Radio);
}

void Panel::gaugeSliderChores() {
    // ------------------------------------------------------------------------
    // gauge and slider
    // ------------------------------------------------------------------------
    ...
    wxStaticBoxSizer *gauge_sizer = new wxStaticBoxSizer( wxHORIZONTAL, m_panel, wxT("&wxGauge and wxSlider") );
    gauge_page_first_row_sizer->Add( gauge_sizer, 0, wxALL, 5 );
    wxBoxSizer *sz = new wxBoxSizer( wxVERTICAL );
    gauge_sizer->Add( sz );
    m_gauge = new wxGauge( m_panel, wxID_ANY, 200, wxDefaultPosition, wxSize(155, 30), wxGA_HORIZONTAL|wxNO_BORDER );
    sz->Add( m_gauge, 0, wxALL, 10 );
    m_slider = new wxSlider( m_panel, ID_SLIDER, 0, 0, 200,
                             wxDefaultPosition, wxSize(155,wxDefaultCoord),
                             wxSL_AUTOTICKS | wxSL_LABELS);
    m_slider->SetTickFreq(40);
    sz->Add( m_slider, 0, wxALL, 10 );

    m_gaugeVert = new wxGauge( m_panel, wxID_ANY, 100,
    ...
    wxStaticBox *sb = new wxStaticBox( m_panel, wxID_ANY, wxT("&Explanation"),
...
#else
    m_wrappingText = new wxStaticText( m_panel, wxID_ANY,
    ...
    wxStaticBoxSizer *non_wrapping_sizer = new wxStaticBoxSizer( wxVERTICAL, m_panel, wxT("Non-wrapping") );
    gauge_page_first_row_sizer->Add( non_wrapping_sizer, 0, wxALL, 5 );

    m_nonWrappingText = new wxStaticText( m_panel, wxID_ANY,
    ...
    wxString s;
    s << initialSpinValue;
    m_spintext = new wxTextCtrl( m_panel, wxID_ANY, s );
    gauge_page_second_row_sizer->Add( m_spintext, 0, wxALL, 5 );

#if wxUSE_SPINBTN
    m_spinbutton = new wxSpinButton( m_panel, ID_SPIN );
...
#if wxUSE_SPINCTRL
    m_spinctrl = new wxSpinCtrl( m_panel, ID_SPINCTRL, wxEmptyString );
...
#if wxUSE_SPINBTN
#if wxUSE_PROGRESSDLG
    m_btnProgress = new wxButton( m_panel, ID_BTNPROGRESS, wxT("&Show progress dialog") );
    gauge_page_second_row_sizer->Add( m_btnProgress, 0, wxALL, 5 );
#endif // wxUSE_PROGRESSDLG
#endif // wxUSE_SPINBTN

    wxButton* newTextButton = new wxButton( m_panel, ID_BTNNEWTEXT, wxT("New text"));
    gauge_page_second_row_sizer->Add( newTextButton, 0, wxALL, 5 );

    gauge_page_vsizer->Add(gauge_page_second_row_sizer, 1);
    m_panel->SetSizer( gauge_page_vsizer );

    m_book->AddPage(m_panel, wxT("wxGauge"), false, Image_Gauge);
	
#endif // wxUSE_SLIDER && wxUSE_GAUGE
}

void Panel::bitmapChores() {
    // ------------------------------------------------------------------------
    // wxBitmapXXX
    // ------------------------------------------------------------------------
    m_panel = new wxPanel(m_book);
    ...
    (void) new wxStaticBitmap( m_panel, wxID_ANY, icon, wxPoint(10, 10) );
    ...
    wxPanel *panel2 = new wxPanel(m_panel, -1, wxPoint(100, 0), wxSize(100, 200));
    ...
    if(bitmap.IsOk()) {
       ...
       (void)new wxStaticBitmap(m_panel,  wxID_ANY, bitmap, wxPoint(300, 120));
    }
#endif
    ...
    (void)new wxToggleButton(m_panel, ID_BUTTON_LABEL,
                             wxT("&Toggle label"), wxPoint(250, 20));

    m_label = new wxStaticText(m_panel, wxID_ANY, wxT("Label with some long text"),
                               wxPoint(250, 60), wxDefaultSize,
                               wxALIGN_RIGHT /*| wxST_NO_AUTORESIZE*/);
    m_label->SetForegroundColour( *wxBLUE );

    m_book->AddPage(m_panel, wxT("wxBitmapXXX"));
}

void Panel::sizerChores() {
    // ------------------------------------------------------------------------
    // sizer page
    // ------------------------------------------------------------------------
    ...
    wxStaticBoxSizer *csizer =
      new wxStaticBoxSizer (new wxStaticBox (m_panel, wxID_ANY, wxT("Show Buttons")), wxHORIZONTAL );

    wxCheckBox *check1, *check2, *check3, *check4, *check14, *checkBig;
    check1 = new wxCheckBox (m_panel, ID_SIZER_CHECK1, wxT("1"));
    check1->SetValue (true);
    csizer->Add (check1);
    check2 = new wxCheckBox (m_panel, ID_SIZER_CHECK2, wxT("2"));
    check2->SetValue (true);
    csizer->Add (check2);
    check3 = new wxCheckBox (m_panel, ID_SIZER_CHECK3, wxT("3"));
    check3->SetValue (true);
    csizer->Add (check3);
    check4 = new wxCheckBox (m_panel, ID_SIZER_CHECK4, wxT("4"));
    check4->SetValue (true);
    csizer->Add (check4);
    check14 = new wxCheckBox (m_panel, ID_SIZER_CHECK14, wxT("1-4"));
    check14->SetValue (true);
    csizer->Add (check14);
    checkBig = new wxCheckBox (m_panel, ID_SIZER_CHECKBIG, wxT("Big"));
    ...
    m_sizerBtn1 = new wxButton(m_panel, wxID_ANY, wxT("Test Button &1 (tab order 1)") );
    m_buttonSizer->Add( m_sizerBtn1, 0, wxALL, 10 );
    m_sizerBtn2 = new wxButton(m_panel, wxID_ANY, wxT("Test Button &2 (tab order 3)") );
    m_buttonSizer->Add( m_sizerBtn2, 0, wxALL, 10 );
    m_sizerBtn3 = new wxButton(m_panel, wxID_ANY, wxT("Test Button &3 (tab order 2)") );
    m_buttonSizer->Add( m_sizerBtn3, 0, wxALL, 10 );
    m_sizerBtn4 = new wxButton(m_panel, wxID_ANY, wxT("Test Button &4 (tab order 4)") );
    ...
    m_bigBtn = new wxButton(m_panel, wxID_ANY, wxT("Multiline\nbutton") );
    ...
    m_book->AddPage(m_panel, wxT("wxSizer"));
