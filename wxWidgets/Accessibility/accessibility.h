...
class FrameAccess : public wxFrame {
public:
    // ctor(s)
    FrameAccess(const wxString &title, const wxPoint &pos, const wxSize &size,
...
// ----------------------------------------------------------------------------
wxBEGIN_EVENT_TABLE(FrameAccess, wxFrame)
    EVT_MENU(AccessTest_Quit,  FrameAccess::OnQuit)
    EVT_MENU(AccessTest_Query, FrameAccess::OnQuery)
    EVT_MENU(AccessTest_About, FrameAccess::OnAbout)
wxEND_EVENT_TABLE()
// ============================================================================
// implementation
// ============================================================================
...
// main frame
// ----------------------------------------------------------------------------
FrameAccess::FrameAccess(const wxString &title, const wxPoint &pos, const wxSize &size, long style)
...
// *****************<<_EVENT HANDLES_>>*********************
void FrameAccess::OnAbout(wxCommandEvent& WXUNUSED(event)) {
    wxString msg;
...
void FrameAccess::OnQuery(wxCommandEvent& WXUNUSED(event)) {
    m_textCtrl->Clear();
...
// Log messages to the text control
void FrameAccess::Log(const wxString& text) {
    if(m_textCtrl) {
        wxString text2(text);
        text2.Replace(wxT("\n"), wxT(" "));
...
// Recursively give information about an object
void FrameAccess::LogObject(int indent, IAccessible* obj) {
    wxString name, role;
    if(indent == 0) {
...
// Get info for a child (id > 0) or object (id == 0)
void FrameAccess::GetInfo(IAccessible *accessible, int id, wxString& name, wxString& role) {
    VARIANT var;
    VariantInit(& var);
    var.vt = VT_I4;
    var.lVal = id;
