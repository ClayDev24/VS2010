...
//*****************************************************************************
void clb::Console::boundaries(int &_x, int &_y) {
	GetConsoleScreenBufferInfo(hOut, &BSInfo);
	_x = BSInfo.srWindow.Right;
	_y = BSInfo.srWindow.Bottom;
}
//*****************************************************************************
void clb::Console::setMinMax(int &_x, int &_y, const int &_mode) {
...
void clb::_console() {
	...
	Console cmd(100, 20, "Console Main");
	_getche();
	//cmd.testeSize();
...
//	cmd.printConsole("Teclado habilitado para o porguguês!", 10, 10);
	...
	cmd.write2Console("Hey I'm here! hahaha", 3, 3);
	_getche();
